...
{ 
    public class TverdToplivo
    {
        ...
        #region Тепло, полезно затраченное на выработку пара Q_k, кВт
        ...
        /// <summary>
        /// Энтальпия нагретого пара, Дж/кг
        /// </summary>
        ...
        public double EntBoilWat()
        {
            ...
            double _entBoil = 2d * Math.Pow(10, -7) * Math.Pow(_tempBoil,2) + Math.Pow(10, -4) * _tempBoil + 1.495;//энтальпия кипятка
            _entBoilWat = _tempBoil * _entBoil;
            return _entBoilWat;
        }

        /// <summary>
        /// Тепло, полезно затраченное на выработку пара Q_k, кВт
        /// </summary>
        ...
        #endregion Тепло, полезно затраченное на выработку пара Q_k, кВт
        ...
        {        
            double _teploemk = 7d * Math.Pow(10,-7) * Math.Pow(_tempRabT,2) + 13d * Math.Pow(10, -4) * _tempRabT + 0.9575; //теплоемкость рабочего топлива
        ...
        {
            _warmBurnLow = 339d * _sodC + 1030d * _sodH - 109d * (_sodO + _sodS) - 25.14 * (9d * _sodH + _sodWP);
            return _warmBurnLow;
        }
        ...
        {
            double sum = (_co2 * _burnco2  + _h2o * _burnh2o) + _burnn2 * _n2 + _burno2 * _o2+ _burnso2 * _so2;//выкинула so2 тк =0,бессмысленно
        ...
        /// <summary>
        /// Потери теплоты с физическим шлаком (нигде не используется), кДж/кг
        /// </summary>
